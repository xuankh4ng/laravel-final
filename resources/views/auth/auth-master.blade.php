...
<body class="auth-body">
  ...
  <div class="container-box {{ (request()->routeIs('register') || $errors->has('name') || $errors->has('phone') || $errors->has('password_confirmation')) ? 'right-panel-active' : '' }}" id="container">
    ...
    <div class="form-container sign-up-container">
        <form action="{{ route('register') }}" method="POST">
            ...
            @error('name') <span class="text-red-500 text-xs self-start ml-2">{{ $message }}</span> @enderror
            ...
            <input type="text" name="phone" placeholder="0123456789" value="{{ old('phone') }}" required />
            @error('phone') <span class="text-red-500 text-xs self-start ml-2">{{ $message }}</span> @enderror

            <label class="form-label">Mật khẩu</label>
            <input type="password" name="password" placeholder="......." required />
            @error('password') <span class="text-red-500 text-xs self-start ml-2">{{ $message }}</span> @enderror
            
            <input type="password" name="password_confirmation" placeholder="Nhập lại mật khẩu" class="mt-2" required />
    ...
    <div class="form-container sign-in-container">
        <form action="{{ route('login') }}" method="POST">
            ...
            <a href="#" class="text-sm text-gray-500 hover:text-gray-800 mt-4 mb-2">Quên mật khẩu?</a>
